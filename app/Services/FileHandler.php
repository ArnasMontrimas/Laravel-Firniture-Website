...
class FileHandler {
    static function validateFile($request) {
        ...
        if($request->hasFile('productImage')) {
            if($request->file('productImage')->isValid()) {
                ...
                $path = $request->productImage->storeAs('images/', $imageToStore);
            }
        }
    ...
    static function deleteOldFile($product) {
        //Delete Old File
        if(file_exists(public_path("storage/images/$product->img")) && $product->img != 'noimage.jpg') {
            unlink(public_path("storage/images/$product->img"));
        }
    }
