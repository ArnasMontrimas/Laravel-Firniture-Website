...
class ExcelData {
    public static function getExcelData($date1, $date2) {
        $excelData = DB::select("SELECT 
            ...
            WHERE t.product_id = p.id AND t.employee_id = e.id
            AND t.weekEnding BETWEEN ? AND ?
            AND p.status = ? GROUP BY p.id, p.name, p.type, t.employee_id, p.build_time, p.selling_price, e.salary",
